...
test('getShuffledRange - returns as per min, max', () => {
  const answer = Generator.getShuffledRange(2, 10, [], 100);
  expect(answer).toEqual(expect.arrayContaining([2, 3, 4, 5, 6, 7, 8, 9, 10]));
});

test('getShuffledRange - returns as per excludes', () => {
  const answer = Generator.getShuffledRange(2, 10, [4, 8], 100);
  expect(answer).toEqual(expect.not.arrayContaining([4, 8]));
});
